            ...
            <div class="admin-content">
                ...
                <div class="am-tabs am-margin" id="add_tabs" data-am-tabs="{noSwipe: 1}">
                    ...
                    <div class="am-tabs-bd">
                        <div class="am-margin am-text-warning">注意：不需要填的请输入一个 减号 代替，如果是分录题，答案必须填写 空格 或 减号 </div>
                ...
                <div class="am-g am-margin-top-sm am-form-group">
                    ...
                    <div class="am-u-md-6 am-u-end">
                        <select  name="type" id="q_type" required>
                            ...
                            <option value="3" <%if(type==3){%> selected="selected" <%}%> >案例分析</option>
                            <option value="4" <%if(type==4&&right!='-'){%> selected="selected" <%}%> >计算分析</option>
                            <option value="5" <%if(type==4&&right=='-'){%> selected="selected" <%}%> >分录计算</option>
                        </select>
                    </div>
            ...
            if(suit==''||subject==''){
                ...
                if(num!=''){
                    $(".back").on("click",function(){
                        location.href = "exam_p2.php?subject="+subject+"&suit="+suit+"&num="+num;
                    });
                }else{
                    $(".back").on("click",function(){
                        location.href = "exam.php?subject="+subject+"&suit="+suit;
                    });
                }
                if(id!=''){
                    request({method:"getExam",subject:subject,suit:suit,id:id,num:num},function(d){
...
function addValid(){
    ...
    submit:function(e){
        if(this.isFormValid()){
            var p = {method:"exam",id:id,subject:subject,suit:suit,parent:num};
            $.each($('#q_form').serializeArray(),function(i,v){
                if(v.name=='type'&&v.value=='5'){//分录
                    v.value = '4';
                    p.p1 = "1";
                }
                p[v.name] = v.value;
            ...
            request(p,function(data){
                messageAlert("保存成功",function(){location.reload(true);});
            });
        }
        return false;
    }
